...
func (z *Zip) Archive(sources []string, destination string) error {
	...
	if !z.OverwriteExisting && common.FileExists(destination) {
	...
	if z.MkdirAll && !common.FileExists(destDir) {
		err := common.Mkdir(destDir, 0o755)
		if err != nil {
	...
	if z.ImplicitTopLevelFolder && common.MultipleTopLevels(sources) {
...
func (z *Zip) Unarchive(source, destination string) error {
	if !common.FileExists(destination) && z.MkdirAll {
		err := common.Mkdir(destination, 0o755)
		if err != nil {
	...
	if z.ImplicitTopLevelFolder {
		...
		if common.MultipleTopLevels(files) {
			destination = filepath.Join(destination, common.FolderNameFromFileName(source))
		}
	}
	...
	for {
		...
		if err != nil {
			if z.ContinueOnError || common.IsIllegalPathError(err) {
				log.Printf("[ERROR] Reading file in zip archive: %v", err)
				continue
			}
			return fmt.Errorf("reading file to %q in zip archive: %v", destination, err)
		}
	}
...
func (z *Zip) extractFile(f common.File, to string, header *zip.FileHeader) error {
	...
	if !z.OverwriteExisting && common.FileExists(to) {
		return fmt.Errorf("file already exists: %s", to)
	}

	// extract symbolic links as symbolic links
	if common.IsSymlink(header.FileInfo()) {
		...
		return common.WriteNewSymbolicLink(to, strings.TrimSpace(buf.String()))
	}
...
func (z *Zip) Write(f common.File) error {
	if z.zw == nil {
...
func (z *Zip) writeFile(f common.File, writer io.Writer) error {
	if f.IsDir() {
...
func (z *Zip) Read() (common.File, error) {
	if z.zr == nil {
		return common.File{}, fmt.Errorf("zip archive is not open")
	}
	if z.ridx >= len(z.zr.File) {
		return common.File{}, io.EOF
	}
...
func (z *Zip) Walk(archive string, walkFn common.WalkFunc) error {
...
func (z *Zip) Extract(source, target, destination string) error {
	...
	return z.Walk(source, func(f common.File) error {
		...
		if common.Within(target, zfh.Name) {
...
// New returns a new, default instance ready to be customized and used.
func New() *Zip {
	return &Zip{
